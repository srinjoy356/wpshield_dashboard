...
function isCveVulnerable(
    ...
    for (const node of config.nodes ?? []) {
      for (const match of node.cpeMatch ?? []) {
        ...
        if (
          ...
          productNormalized.includes(slugNormalized) ||
          slugNormalized.includes(productNormalized)
        ) {
          matchedCpe = true;
          let startOk = true,
            endOk = true;
          if (match.versionStartIncluding)
            startOk =
              compareVersions(currentVersion, match.versionStartIncluding) >= 0;
          if (match.versionStartExcluding)
            startOk =
              startOk &&
              compareVersions(currentVersion, match.versionStartExcluding) > 0;
          if (match.versionEndIncluding)
            endOk =
              compareVersions(currentVersion, match.versionEndIncluding) <= 0;
          if (match.versionEndExcluding)
            endOk =
              endOk &&
              compareVersions(currentVersion, match.versionEndExcluding) < 0;
          if (
  ...
  if (!matchedCpe) {
    ...
    if (
      ...
      (lowerDesc.includes(slug.toLowerCase()) ||
        lowerDesc.includes(slug.toLowerCase().replace("-", " ")))
    ) {
      const upToMatch = lowerDesc.match(
        /(?:up to|through|including|before|prior to)\s+([0-9]+\.[0-9]+(?:\.[0-9]+)?)/,
      );
      if (upToMatch) {
        return compareVersions(currentVersion, upToMatch[1]) <= 0;
      }
      return true;
    }
  }
...
async function runVulnCheck() {
  ...
  for (const company of companies) {
    ...
    for (const target of targets) {
      const { site_id, url: targetUrl } = target;

      // Latest plugins snapshot for THIS specific site — querying by company_id alone
      // (the old behavior) meant a company with two sites only ever saw whichever
      // site happened to report most recently, and the other site's plugins were
      // never vulnerability-checked at all. Sites that predate migration 020 (no
      // site_id recorded on their snapshots yet) fall back to a site_url match.
      ...
      snapshotQuery = site_id
        ? snapshotQuery.eq("site_id", site_id)
        : snapshotQuery.eq("site_url", targetUrl);
      ...
      const payload =
        typeof snapshots[0].payload === "string"
          ? JSON.parse(snapshots[0].payload)
          : snapshots[0].payload;
      ...
      const pluginVulnMap = new Map<
        string,
        { name: string; version: string; vulns: any[] }
      >();
      ...
      for (const plugin of plugins) {
        ...
        try {
          ...
          if (wpRes.ok) {
            ...
            for (const vuln of data[slug]?.vulnerabilities ?? []) {
              ...
              if (!fixedIn || compareVersions(version, fixedIn) < 0) {
                ...
                const refUrl =
                  vuln.references?.url?.[0] ??
                  `https://wpscan.com/vulnerability/${vuln.id}`;
                let cvssScore = null,
                  severity = "medium";
                if (vuln.cvss?.score) {
          ...
          console.error(`WPScan failed for ${slug}:`, err);
        }
        ...
        try {
          ...
          if (nvdRes.ok) {
            ...
            for (const item of data.vulnerabilities ?? []) {
              ...
              const cveId = cve.id;
              if (
                Array.from(foundVulnsMap.values()).some(
                  (v) => v.cveId === cveId,
                )
              )
                continue;
              ...
              const severity =
                cvssData?.baseSeverity?.toLowerCase() || "medium";
              let fixedIn = null;
          ...
          console.error(`NVD failed for ${name}:`, err);
        }
      ...
      // Grouped alert per plugin — title includes the specific site URL so the same
      // vulnerable plugin on two different sites under one company produces two
      // distinguishable alerts rather than colliding on title + company_id alone.
      for (const [, pluginData] of Array.from(pluginVulnMap.entries())) {
        const { name, version, vulns } = pluginData;
        const alertTitle = `Vulnerable plugin detected: ${name} (${targetUrl})`;
        let existingAlertQuery = supabase
          ...
          .eq("title", alertTitle)
          .eq("status", "open");
        existingAlertQuery = site_id
          ? existingAlertQuery.eq("site_id", site_id)
          : existingAlertQuery.is("site_id", null);
        const { data: existing } = await existingAlertQuery.maybeSingle();
...
export async function GET(request: Request) {
  ...
  runVulnCheck().catch((err) =>
    console.error("vuln-check background error:", err),
  );

  return NextResponse.json({
    success: true,
    message: "Vuln check started in background",
  });
}